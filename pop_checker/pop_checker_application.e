...
--			l_pop3_location: POP3_LOCATION
	...
	check_this_profile (a_uuid: STRING_8)
		...
		do
			p := mail_checker_data.profile (a_uuid)
			if p /= Void then
				check_profile (p)
			end
		end
			...
			if attached a_mail_account.messages_by_index as l_messages then
				...
				loop
					if attached l_messages[i] as l_mesg then
						...
						if attached l_mesg.header_subject as l_text then
							s := l_text.string
						...
						if attached l_mesg.header_from as l_text then
							s := l_text.string
							s.replace_substring_all ("<", "&lt;")
							s.replace_substring_all (">", "&gt;")
						...
						if attached l_mesg.header_to as l_text and then l_text.count > 0 then
							s := l_text.string
							s.replace_substring_all ("<", "&lt;")
							s.replace_substring_all (">", "&gt;")
